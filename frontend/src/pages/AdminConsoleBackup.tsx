...
const initialFields: AccessField[] = [
  {
    id: "status",
    label: "Verified / not verified status",
    desc: "Always shareable — core purpose of the network",
    enabled: true,
    locked: true,
  },
...
export default function AdminConsole() {
  ...
  const [whitelisted, setWhitelisted] =
    useState<Institution[]>(initialWhitelisted);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto py-10 px-6 space-y-6">
        ...
        <SectionCard
          ...
          description="Awaiting approval to join the network."
        >
          {pending.length === 0 ? (
            ...
            <div className="space-y-3">
              {pending.map((p) => (
                ...
                >
                  ...
                  <Button variant="danger" onClick={() => reject(p)}>
                    Reject
                  </Button>
                  <Button variant="success" onClick={() => approve(p)}>
                    Approve
                  </Button>
                </div>
              ))}
        ...
        <SectionCard
          title="Whitelisted institutions"
          description="Currently allowed to request customer KYC."
        >
          <div className="space-y-3">
            {whitelisted.map((w) => (
              <div
              ...
              >
                ...
                <Button variant="danger" onClick={() => suspend(w)}>
                  Suspend
                </Button>
              </div>
            ))}
        ...
        <SectionCard
          title="Data field access rules"
          description="Fields institutions are permitted to request."
        >
          <div className="divide-y divide-line -mt-2">
            {fields.map((f) => (
              ...
              >
                ...
                >
                  <span
                    className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${f.enabled ? "translate-x-4" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
            ))}
        ...
        <SectionCard
          ...
          description="All verification activity, across every institution."
        >
          <div className="divide-y divide-line -mt-2">
            {auditLog.map((a, i) => (
